...
-- ═══════════════════════════════════════════
--              INTERACTABLES PANEL (EVIL EDITION)
-- ═══════════════════════════════════════════
...
InteractablesScroll.ScrollBarImageColor3 = Color3.fromRGB(156, 39, 176)
...
mainContainer.Parent = InteractablesScroll
...
mainLayout:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(function()
    ...
    InteractablesScroll.CanvasSize = UDim2.new(0, 0, 0, mainLayout.AbsoluteContentSize.Y + 20)
end)
...
-- Evil‑themed teleport button
local function MakeInteractableButton(displayText, onClick)
    local btn = Instance.new("TextButton")
...
-- Collapsible section (reused from Teleport panel)
local function CreateCollapsibleSection(parent, title, icon)
    ...
    local function UpdateContentHeight()
        ...
        return buttonCount == 0 and 60 or math.min(buttonCount * 40, 200)
    end
...
-- Data storage for categories (excluding Interactable)
local categories = {
    FriendlyNpcs = { folder = nil, items = {}, container = nil, refresh = nil, icon = "👥" },
    Enemies      = { folder = nil, items = {}, container = nil, refresh = nil, icon = "👹" },
    AreaParts    = { folder = nil, items = {}, container = nil, refresh = nil, icon = "🗺️" },
    Live         = { folder = nil, items = {}, container = nil, refresh = nil, icon = "👤" },
}

-- Helper to get position (same as GetTeleportPosition but using existing function)
local function GetInteractablePos(obj)
...
local function RefreshCategory(catName)
    ...
    if folder then
        for _, obj in ipairs(folder:GetChildren()) do
            if obj:IsA("Model") or obj:IsA("BasePart") then
                local name = obj.Name
                local pos = GetInteractablePos(obj)
                if pos then
    ...
    for _, item in ipairs(newItems) do
        local btn = MakeInteractableButton("📍 " .. item.name, function()
...
for catName, cat in pairs(categories) do
    local container, _, refreshFunc = CreateCollapsibleSection(mainContainer, catName, cat.icon)
...
task.spawn(function()
    while InteractablesPanel and InteractablesPanel.Parent do
        task.wait(5)
        if InteractablesPanel.Visible then
            for catName, _ in pairs(categories) do
                RefreshCategory(catName)
            end
        end
