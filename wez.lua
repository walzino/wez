...
local CircleSpeedInput = MakeNumberInput(MoveSection, "Circle Speed", 0.1, 0.05, 1, 54, function(v) FarmerSettings.CircleSpeed = v end)
...
-- AutoFarmer Helper Functions (using existing _GetChar etc.)
local function GetAutoChar() return _GetChar() end
local function GetAutoHRP() return _GetHRP() end
local function GetAutoHumanoid() return _GetHum() end
...
local function UpdateHPBarAuto()
    ...
    local width = math.clamp(hp / 100, 0, 1)
...
local function CircleTargetAuto(dt)
    if not AutoFarmer.CurrentTarget or not AutoFarmer.CurrentTarget.Parent then return false end
    ...
    if not targetPart then targetPart = AutoFarmer.CurrentTarget end
    
    local targetPos = targetPart.Position
    local currentPos = hrp.Position
    local desiredDistance = FarmerSettings.CircleRadius
    ...
    local circleSpeed = FarmerSettings.CircleSpeed
    local time = tick()
